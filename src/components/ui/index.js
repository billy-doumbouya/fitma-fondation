...
export const Button = forwardRef(function Button(
  ...
  const v = {
    savane: "btn btn-savane",
    cauri: "btn btn-cauri",
    nuit: "btn btn-nuit",
    outline: "btn btn-outline-savane",
    ghost: "btn btn-ghost rounded-xl",
...
export const Input = forwardRef(function Input(
  ...
  return (
    <div className="w-full">
      ...
      {!error && helperText && (
        <p className="text-xs mt-1.5" style={{ color: "var(--brume)" }}>
          {helperText}
        </p>
      )}
    </div>
  ...
  return (
    <div className="w-full">
      ...
      <textarea
        ...
        rows={props.rows || 5}
        required={required}
        className={cn("input-f resize-none", error && "err", className)}
        {...props}
      />
      {error && (
        <p
    ...
    >
      {pretitle && (
        <motion.span
          ...
          style={{
            background: light ? "rgba(249,168,37,.25)" : "var(--cauri-pale)",
            color: light ? "var(--cauri-l)" : "var(--savane-d)",
      ...
      <motion.h2
        ...
        style={{ color: light ? "white" : "var(--encre)" }}
      >
        {title}
      </motion.h2>
      {align === "center" && (
  ...
  return (
    <div className="flex items-center justify-center">
      <div
        ...
        style={{ borderColor: "var(--savane)", borderTopColor: "transparent" }}
      />
    </div>
  );
}
...
export function EmptyState({ icon = "📭", title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center px-4">
      ...
      <h3
        className="font-d text-xl font-bold mb-2"
        style={{ color: "var(--encre)" }}
      >
        {title}
      </h3>
      {description && (
        <p className="text-sm max-w-xs" style={{ color: "var(--ardoise)" }}>
          {description}
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={`relative z-10 w-full ${maxW} rounded-3xl p-6`}
        style={{ background: "white", boxShadow: "var(--sh-lg)" }}
        onClick={(e) => e.stopPropagation()}
      >
        {title && (
          <h2
            className="font-d text-xl font-bold mb-4"
            style={{ color: "var(--encre)" }}
          >
            {title}
          </h2>
...
export function ConfirmDialog({
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} size="sm">
      <p className="text-sm mb-6" style={{ color: "var(--ardoise)" }}>
        {message}
      </p>
    ...
    <div className="flex items-center justify-center gap-2 mt-8">
      ...
      {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
        <button
          ...
          style={{
            background: p === page ? "var(--savane)" : "transparent",
            color: p === page ? "white" : "var(--ardoise)",
          }}
        >
          {p}
        </button>
  ...
  return (
    <div className="w-full">
      {label && (
        ...
        >
          <span>{label}</span>
          <span className="font-semibold" style={{ color: "var(--savane)" }}>
            {value}%
          </span>
        </div>
      )}
...
export function StatCard({ icon: Icon, label, value, sub, couleur, bg, href }) {
  const inner = (
    <div className="stat-card">
      ...
      <p
        className="font-d text-2xl font-bold leading-tight"
        style={{ color: "var(--encre)" }}
      >
        {value}
      </p>
