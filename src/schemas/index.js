...
export const loginSchema = yup.object({
  ...
  motDePasse: yup.string().required("Mot de passe obligatoire").min(6, "Minimum 6 caractères"),
})
...
export const inscriptionSchema = yup.object({
  ...
  motDePasse:  yup.string().required("Mot de passe obligatoire").min(8, "Minimum 8 caractères"),
  confirmerMdp:yup.string().oneOf([yup.ref("motDePasse")], "Les mots de passe ne correspondent pas").required(),
})
...
export const donSchema = yup.object({
  montant:           yup.number().required("Montant obligatoire").positive().min(10000, "Minimum 10 000 GNF"),
  montantCustom:     yup.number().optional().positive(),
  prenom:            yup.string().required("Prénom obligatoire").min(2),
  nom:               yup.string().required("Nom obligatoire").min(2),
  email:             yup.string().required("Email obligatoire pour votre reçu").email("Email invalide"),
  telephone:         yup.string().optional(),
  type:              yup.string().oneOf(["UNIQUE","MENSUEL"]).default("UNIQUE"),
  accepteConditions: yup.boolean().oneOf([true], "Veuillez accepter les conditions"),
})

export const candidatureSchema = yup.object({
  nomProjet:           yup.string().required("Nom du projet obligatoire").min(3),
  descriptionProjet:   yup.string().required("Description obligatoire").min(50, "Décrivez votre projet en au moins 50 caractères"),
...
export const formationAdminSchema = yup.object({
  ...
  niveau:      yup.string().oneOf(["DEBUTANT","INTERMEDIAIRE","AVANCE"]).required(),
